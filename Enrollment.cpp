...
ON_BN_CLICKED(ID_CANCEL_BUTTON, &Enrollment::OnBnClickedCancelButton)
//ON_BN_CLICKED(IDOK, &Enrollment::OnBnClickedOk)
END_MESSAGE_MAP()
...
void Enrollment::OnBnClickedSubjectSelect()
{
	...
	if (nIndex != LB_ERR) {
		...
		subject_combo.DeleteString(nIndex);
...
{
	int nIndex = cart_list.SetCurSel(0);
	int cnt = cart_list.GetCount();
...
void Enrollment::OnBnClickedCancelButton()
{
	int nIndex = enroll_list.GetCurSel();
	CString str;
	enroll_list.GetText(nIndex, str);
	enroll_list.DeleteString(nIndex);
	subject_combo.AddString(str);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}
...
//{
//	CSWStudentMakerView dlg;
//	CString str;
//	int nIndex = enroll_list.GetCurSel;
//	int cnt = enroll_list.GetCount();
//	if (nIndex != LB_ERR) {
//		for (int i = 0; i < cnt;i++)
//			
//	}
